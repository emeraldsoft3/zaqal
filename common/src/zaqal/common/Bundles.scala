...
class PreDecodeSignals extends Bundle {
  ...
  val is_call = Bool() // Call hint (rd == x1 || rd == x5)
  val is_ret  = Bool() // Return hint (rs1 == x1 || rs1 == x5)
...
class DecodeSignals(implicit val p: Parameters) extends Bundle with HasZaqalParameter {
  ...
  val is_fmv_d_x  = Bool()
  val is_fmv_x_d  = Bool()
  val is_fcvt_s_d = Bool()
  val is_fcvt_d_s = Bool()
  val is_fp_double = Bool()
  val is_feq      = Bool()
  val is_flt      = Bool()
  val is_fle      = Bool()
  val is_fclass   = Bool()
  ...
  val fused_alu_op = UInt(3.W)
  val fused_type = UInt(5.W)
...
object FusionType {
  ...
  // Additional XiangShan Specialized Bitmanip Idioms:
