...
- [x] **Day 25**: **Memory Dependence Predictor (MDP)**: Implement Store Sets (SSIT/LFST) or Wait Table to predict load/store collisions and avoid costly memory violation flushes (XiangShan parity).
- [x] **Day 25.5**: **RV64D (Double Precision FPU)**: Complete the "G" extension requirement by adding 64-bit double precision paths to the FPU execution units, missing from Phase 3.
- [x] **Day 25.8**: **Advanced Micro-op Fusion**: Implement advanced fusion rules (e.g. Memory + ALU) in the decoders and issue queues.
...
- [x] **Day 34-35**: **Prefetch Coordination**: Throttling for bus congestion and L2-cache interaction.
