...
class IssueQueue(val numEntries: Int, val numEnq: Int, val numDeq: Int, val numWakeup: Int)(implicit val p: Parameters) extends Module with HasZaqalParameter {
  ...
  ageDetector.io.canIssue(0) := can_issue.asUInt

  val issue_onehot = ageDetector.io.out(0)
  val issue_idx = OHToUInt(issue_onehot)
  val issue_valid = issue_onehot.orR

  io.deq(0).valid := issue_valid
  io.deq(0).bits := entries(issue_idx).uop
  
  val deq_fire = io.deq(0).valid && io.deq(0).ready

  val is_empty = Wire(Vec(numEntries, Bool()))
  for (i <- 0 until numEntries) {
    is_empty(i) := !entries(i).valid || (deq_fire && issue_onehot(i))
  }
